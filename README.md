...
Accounting 是一个极简本地记账 iOS App，当前版本为 `1.1`。项目由 Xcode 创建，主工程位于 `Accounting/Accounting.xcodeproj`，后续开发都在该目录下进行。
...
首页的最近明细和统计页的明细列表中，每条记录都会在用途 / 来源旁显示一个分类标签：

- 收入记录显示“收入”
- 支出记录显示对应支出分类，例如餐饮、购物、交通、居住、娱乐、医疗、学习办公、人情、其他

支出分类标签复用支出分类饼状图的分类规则，保证明细展示、统计汇总和筛选条件一致。

### 统计明细筛选

统计页的“明细”区块新增筛选菜单，位于“选择”按钮左侧。
...
筛选只影响当前时间范围内的明细列表，不影响上方的收入总额、支出总额、结余和支出分类饼状图。筛选后仍保留原有明细操作：
...
- `ContentView.swift`：首页、统计页、录入页、编辑页、图表、分类标签、明细筛选和交互
